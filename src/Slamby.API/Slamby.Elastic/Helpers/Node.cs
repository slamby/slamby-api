...
namespace Slamby.Elastic.Helpers
{
    ...
    public class Node<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<Node<T>>
    {
        ...
        public void AddParent(Node<T> parentNode)
        {
            if (!IsRoot)
            {
                throw new ArgumentException("This node [{0}] already has a parent".FormatInvariant(Value), nameof(parentNode));
            }
            parentNode.Add(this);
        }
        ...
        public static IEnumerable<Node<T>> CreateTree<TId>(IEnumerable<T> values, Func<T, TId> idSelector, Func<T, TId> parentIdSelector)
        {
            ...
            if (!Equals(itemWithIdAndParentIdIsTheSame, default(T)))
            {
...

        {
            ...
            foreach (var rootNode in rootNodesCache)
            {
                ...
                else if (!Equals(parentId, default(TId)))
                {
                    throw new ArgumentException("A value has the parent ID [{0}] but no other nodes has this ID".FormatInvariant(parentId));
                }
            }
